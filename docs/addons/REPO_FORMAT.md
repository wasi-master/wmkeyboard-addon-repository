...
| `snippets` | `*.wmsnippets.json` | `{ "format":"wmkeyboard-snippets", "version":1, "snippets":[ { "id":1, "label":"…", "text":"…", "trigger":"…"?, "triggerPattern":"…"?, "triggerWords":3? }, … ] }`. Ids are reassigned on import. `trigger` is one exact word; `triggerPattern` is an anchored regular expression over the last `triggerWords` words before the cursor (default 3, max 8), whose captures reach `text` as `$1`…`$9` — see the app's Snippets documentation. A snippet carrying both keeps the word. Both pattern fields are written only when set, so a pack of plain snippets looks exactly as it always did. |
